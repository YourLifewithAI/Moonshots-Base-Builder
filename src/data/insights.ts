...
export const INSIGHTS: InsightDef[] = [
  ...
  { tech: 'mpptInverters', discount: 0.3, hint: '6 Solar Arrays operating',
    lesson: 'six arrays showed how much each string leaves on the table', check: (s) => activeCount(s, 'solar') >= 6 },
  { tech: 'heatRecoveryJackets', discount: 0.3, hint: '200◆ smelted',
    lesson: 'two hundred smelts of heat went up the stack', check: (s) => s.stats.produced.metals >= 200 },
  { tech: 'neutronSpectrometry', discount: 0.4, hint: '3 prospects surveyed',
    lesson: 'three surveys taught the masts what to listen for', check: (s) => Object.keys(s.survey.prospects).length >= 3 },
  // era 3
  ...
  { tech: 'stackedCells', discount: 0.3, hint: '2 nights with load shed',
    lesson: 'a second short night: the pad needs another tier', check: (s) => s.stats.nightBrownouts >= 2 },
  { tech: 'slagRecycling', discount: 0.3, hint: '600◆ smelted',
    lesson: 'the slag heap is taller than the smelter', check: (s) => s.stats.produced.metals >= 600 },
  { tech: 'refluxColumns', discount: 0.3, hint: '300◇ refined',
    lesson: 'three hundred refines mapped the cut you were losing', check: (s) => s.stats.produced.silicon >= 300 },
  { tech: 'cryoSampleStore', discount: 0.3, hint: '4 labs operating',
    lesson: 'four benches, and the cores go stale waiting', check: (s) => activeCount(s, 'lab') >= 4 },
  // era 4
  ...
  { tech: 'roverAutonomy', discount: 0.3, hint: '2 rovers on one site at once',
    lesson: 'two rovers shared one weld — let them plan it between themselves', check: (s) => (s.stats.crowdedSiteMax ?? 0) >= 2 },
  ...
  { tech: 'braytonConverters', discount: 0.3, hint: '3 nights survived',
  ...
  { tech: 'autonomousHaulage', discount: 0.3, hint: 'an excavator hauling 100 m',
    lesson: 'a long haul road showed where the digger’s minutes go', check: (s) => (s.stats.haulMaxM ?? 0) >= 100 },
  ...
  { tech: 'toolChangers', discount: 0.3, hint: '500⚙ fabricated',
    lesson: 'five hundred parts, and half the shift was changing heads', check: (s) => s.stats.produced.parts >= 500 },
  // era 6
  ...
  { tech: 'rollToRoll', discount: 0.3, hint: '30▰ manufactured',
    lesson: 'thirty foils, and the line waits on its own cure', check: (s) => s.stats.produced.foils >= 30 },
  { tech: 'liquidCooling', discount: 0.3, hint: 'a Data Center has operated 2,160 s',
    lesson: 'three days of racks: the fans are the biggest load in the hall', check: (s) => s.stats.dcOpS >= 2160 },
