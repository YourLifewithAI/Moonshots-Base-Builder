...
export function roadAction(s: GameState, hf: Heights, a: Extract<Action, { kind: 'layRoad' | 'removeRoad' }>) {
  if (a.kind === 'layRoad') {
    ...
    layJob(s, plan, 'draw');
    alert(s, `ROAD LAID — ${cells(plan.cells.length)} for the rovers to sinter (${Math.round(plan.cells.length * ROAD.cellS)} rover-s)`, 'info');
    return;
  }
  ...
  if (!n) return;
  alert(s, cut.length
    ? `ROAD REMOVED — ${cells(n)} · ${cut.slice(0, 3).join(', ')}${cut.length > 3 ? '…' : ''} no longer has a road to its door`
    : `ROAD REMOVED — ${cells(n)}`, cut.length ? 'warn' : 'info');
}
