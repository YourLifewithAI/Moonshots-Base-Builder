...
try {
  await build({
    stdin: {
      contents: `
        export { TECHS, TECH_ORDER, ERA_NAMES, LANES, DOCTRINES, ERA_GATES, TRACKS, describeTech } from './src/data/techs';
...
function renderTechs(T) {
  const { TECHS, TECH_ORDER, ERA_NAMES, LANES, DOCTRINES, ERA_GATES, TRACKS, ERA_COST_SCALE, INSIGHTS, SITES, RESOURCES, PROSPECTS, describeTech } = T;
  ...
  for (let era = 1; era <= 8; era++) {
    ...
    if (g) {
      const pick = era >= 3 ? TRACKS[era - 1] : null;
      L(`Opens with ${T.CHARTER_TECHS} techs of era ${era - 1}, or ${T.CHARTER_DEED_TECHS} plus the deed: **${g.deed}**` +
        (pick ? `; one of them must be era ${era - 1}'s destiny pick, ⌂ ${name(pick.colony)} or ◉ ${name(pick.automation)}` : '') +
        (g.roboticRequires ? ` (robotic runs also need ${name(g.roboticRequires)}, which either Era 6 pick settles)` : '') + '.');
      L();
    }
    if (TRACKS[era]) {
      const t = TRACKS[era];
      L(`**Destiny · ${esc(t.question)}** ⌂ ${name(t.colony)} or ◉ ${name(t.automation)}${era === 1 ? ' — chosen on the landing screen' : ''}.`);
      L();
    }
