...
research) and the D6 ending are on main. **D3 (hazards) is on main too**
(§9, "As shipped (D3)"). **D4 (the look) and D5 (audio) ship on
`work/look`** (§9, "As shipped (D4 and D5)"). As everywhere in these docs, the
...
| Control plane | no DC run for 15 s | leaky darkness too; one warning lasts through a flicker, and it stands down once the darkness has drained. Darkness carried over from a held-back start never cuts the 15 s warning, and a restore (a DC run 30 s) clears it | a brownout that flickered the Data Centers opened and closed a new hazard every few seconds: 67 in one run |
